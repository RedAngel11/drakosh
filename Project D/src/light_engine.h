...
enum class Emotion { CALM, JOY, SUPPORT, ALARM, SLEEP, OFF };
...
public:
  ...
  void setEmotion(Emotion e, bool instant = false) {
    ...
    _startMs = millis();
    if (instant) { _cur = _target; _apply(); _fading = false; }
    else _fading = true;
  }

  // плавный фейд ~1.2 с по ВРЕМЕНИ — не зависит от частоты loop()
  void update() {
...
private:
  void _apply() {
    ...
    for (uint16_t i = 0; i < _strip->numPixels(); i++) _strip->setPixelColor(i, c);
    _strip->show();
  }
